...
public class SACParser implements Parser, SACParserConstants {
	private static final Logger logger = Logger.getLogger(SACParser.class
			.getName());
	...
	public void parseStyleSheet(InputSource source) throws CSSException,
		...
		try {
			...
			// TODO
			System.out
					.println("Erreur de parsing de type CSSException.SAC_SYNTAX_ERR,");
	...
	public void parseStyleDeclaration(InputSource source) throws CSSException,
		...
		try {
			...
			throw new CSSException(CSSException.SAC_SYNTAX_ERR, e.getMessage(),
					e);
		}
	}
	...
	private CharStream getCharStream(InputSource source) throws CSSException,
			IOException {
	...
	final public void mediaListOrExpression(SACMediaListImpl ml)
	...
	final public void mediaExpression(SACMediaListImpl ml)
			throws ParseException {
	...
	final public LexicalUnit operator(LexicalUnit prev) throws ParseException {
		...
		case EQUALS:
			...
			}
			{
				if (true)
					return new LexicalUnitImpl(prev,
							LexicalUnit.SAC_OPERATOR_EQUALS);
			}
			break;
		default:
			jj_la1[58] = jj_gen;
	...
	final public void styleRule() throws ParseException {
		...
		try {
			...
			logger.log(Level.WARNING, "styleRule(): " + _source.getURI()
					+ ": Skipping block: " + e.getMessage() + ".");
	...
	final public Selector simpleSelector(Selector sel, char comb)
		...
		if (sel != null) {
			...
			case ' ':
				...
				break;
			case '+':
				sel = _selectorFactory.createDirectAdjacentSelector(sel
						.getSelectorType(), sel, simpleSel);
				break;
			case '>':
	...
	final public Condition _class(Condition pred) throws ParseException {
		...
		{
			if (true)
				return (pred == null) ? c : _conditionFactory
	...
	final public SimpleSelector elementName() throws ParseException {
		...
		case IDENT:
			...
			{
				if (true)
					return _selectorFactory.createElementSelector(null,
							unescape(t.image));
			}
			break;
		case ASTERISK:
	...
	final public Condition attrib(Condition pred) throws ParseException {
		...
		switch (type) {
		case 0:
			c = _conditionFactory.createAttributeCondition(name, null, false,
					null);
			break;
		case 1:
			c = _conditionFactory.createAttributeCondition(name, null, false,
			...
			break;
		case 2:
			c = _conditionFactory.createOneOfAttributeCondition(name, null,
					false, value);
			break;
		case 3:
			c = _conditionFactory.createBeginHyphenAttributeCondition(name,
					null, false, value);
			break;
		}
		{
	...
	// ;
	//
	final public Condition pseudo(Condition pred) {
		Condition c;
		Token t;
		String function;
		String arg;
		...
		case IDENT:
			try {
				...
				// TODO Sahar
				System.out.println("erreur de parising");
				// e.printStackTrace();
			}
	...
	final public void declaration() throws ParseException {
		...
		try {
			...
			logger
					.log(Level.WARNING, "declaration(): " + _source.getURI()
							+ ": Skipping declaration due to: "
							+ ex.getMessage() + ".");
			if (logger.isLoggable(Level.INFO)) {
	...
	final public LexicalUnit term(LexicalUnit prev) throws ParseException {
		...
		case FUNCTION:
			...
			case PERCENTAGE:
				...
				value = LexicalUnitImpl.createPercentage(prev, value(op,
						t.image));
				break;
			case LENGTH_PX:
			...
			case LENGTH_CM:
				...
				value = LexicalUnitImpl.createCentimeter(prev, value(op,
						t.image));
				break;
			case LENGTH_MM:
				t = jj_consume_token(LENGTH_MM);
				value = LexicalUnitImpl.createMillimeter(prev, value(op,
						t.image));
				break;
			case LENGTH_IN:
			...
			case TIME_MS:
				...
				value = LexicalUnitImpl.createMillisecond(prev, value(op,
						t.image));
				break;
			case TIME_S:
		...
		case DIMEN:
			...
			value = LexicalUnitImpl.createDimension(prev, value(op, t.image
					.substring(0, n + 1)), t.image.substring(n + 1));
			break;
		case INHERIT:
		...
		default:
			jj_la1[104] = jj_gen;
			jj_consume_token(-1);
			System.out.println("exeption de parsing" );
			throw new ParseException();
		}
		label_62: while (true) {
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
	...
	final public LexicalUnit function(LexicalUnit prev) throws ParseException {
		...
		{
			if (true)
				return LexicalUnitImpl.createFunction(prev, t.image.substring(
						0, t.image.length() - 1), params);
		}
		throw new Error("Missing return statement in function");
	}
	...
	void skipSelector() throws ParseException {
		Token t = getToken(1);
		while (t.kind != COMMA && t.kind != SEMICOLON && t.kind != LBRACE
				&& t.kind != EOF) {
			getNextToken();
			t = getToken(1);
		}
	...
	private static void jj_la1_0() {
		jj_la1_0 = new int[] { 0x0, 0x30000002, 0x30000002, 0x0, 0x30000002,
				0x30000002, 0x841a00, 0x841a00, 0x30000002, 0x30000002,
				0x841a00, 0x2, 0x2, 0x2, 0x9000000, 0x2, 0x0, 0x2, 0x2,
				0x841a00, 0x300000, 0x300000, 0x400100, 0x2, 0x100, 0x2,
				0x400000, 0x400100, 0x2, 0x2, 0x800, 0x100, 0x2, 0x841a00, 0x2,
				0x841a00, 0x841a00, 0x2, 0x2, 0x2, 0x2, 0x2, 0x800, 0x800, 0x2,
				0x0, 0x400, 0x2, 0x0, 0x2, 0x2, 0x0, 0x400, 0x2, 0x0, 0x2, 0x2,
				0x2, 0x12100, 0x2, 0x2, 0x24000, 0x2, 0x24000, 0x24002, 0xc000,
				0x2, 0x2, 0x0, 0x400, 0x2, 0x0, 0x100, 0x2, 0x2, 0x840a00,
				0x840a00, 0x840a00, 0x840a00, 0x841a00, 0x200, 0x1000, 0x2,
				0x2, 0xc0010000, 0x2, 0x1000000, 0x2, 0xc0010000, 0x2, 0x2,
				0x0, 0x2, 0x0, 0x400, 0x2, 0x0, 0x2, 0x0, 0x2, 0x981e100,
				0x12100, 0xc000, 0x0, 0x9800000, 0x2, 0x2, 0x2, };
	}

	private static void jj_la1_1() {
		jj_la1_1 = new int[] { 0x10, 0x0, 0x0, 0x1, 0x0, 0x0, 0x1080002e,
				0x1080002e, 0x0, 0x0, 0x1080003f, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x10000000, 0x0, 0x0, 0x10800022, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10800022, 0x0, 0x10800022,
				0x10800022, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10000000, 0x10000000,
				0x0, 0x10000000, 0x0, 0x0, 0x10000000, 0x0, 0x0, 0x10000000,
				0x0, 0x0, 0x10000000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x10000000, 0x0, 0x0, 0x10000000, 0x0,
				0x0, 0x0, 0x800000, 0x800000, 0x800000, 0x800000, 0x10800000,
				0x800000, 0x10000000, 0x0, 0x0, 0x0, 0x0, 0x10000000, 0x0, 0x0,
				0x0, 0x0, 0x18000000, 0x0, 0x10000000, 0x0, 0x0, 0x10000000,
				0x0, 0x40, 0x0, 0x9fffff80, 0x0, 0x0, 0xb7fff00, 0x9fffff80,
				0x0, 0x0, 0x0, };
	}
