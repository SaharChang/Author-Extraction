...
/**
 * The tokenizer takes a text in input and extract a tokenlist. It uses all
 * services function, language dependant routines, SBD algo and POS tagging. The
 * Tokenizer subsumes and extends the "StringTokenizer", a Java built-in
 * utility.
 * 
...
public class Tokenizer {
	 ...
	 * 
	 * @param pi_Language
	 *            The language this Tokenizer will accept
	 * @param pi_DetectSentenceBoundaries
	 *            Wether or not sentences must be detected
	 */
	public Tokenizer(String pi_Language, boolean pi_DetectSentenceBoundaries) {
		...
		System.out.println("TAGGER DEFINI.");
		m_TokenList = new TokenList(pi_DetectSentenceBoundaries,
				NamedEntityTypeEnum.values());
		...
		m_Rules = Canonizer.RULE_LOWERCASE | Canonizer.RULE_EXPAND_LIGATURES
				| Canonizer.RULE_NORMALIZE_PUNCT;
	...
	public void Tokenize(String pi_SourceText) {
		...
		if (Balie.DEBUG_TOKENIZER)
			DebugInfo.Out("Tokenizing");
		...
		for (int i = 0; i != rawTokens.length; ++i) {
			...
			if (strCurrentComposed.equals("")) {
				...
				String[] strTokens = m_LanguageSpecific
						.Decompound(strCurrentComposed);
				...
				for (int j = 0; j != strTokens.length; ++j) {
					...
					int nNumLead = 0;
					while (nNumLead != nStringLength
							&& !Character.isLetterOrDigit(strCurrentDecomposed
									.charAt(nNumLead))) {
						strCurrentRawTok = strCurrentDecomposed.substring(
								nNumLead, nNumLead + 1);
						strCurrentCanonTok = Canonizer.CanonForm(
								strCurrentRawTok,
								Canonizer.RULE_NORMALIZE_PUNCT, m_PunctLookup,
								m_LigatureLookup, m_AccentLookup);

						Token curToken = new Token(strCurrentRawTok,
								strCurrentCanonTok,
								TokenConsts.TYPE_PUNCTUATION, m_PunctLookup,
								m_AccentLookup, m_NumTokens++, m_NumSentences,
								numSkippedWhite, m_NumChars, m_NETypes.length);
						m_NumChars = curToken.EndPos();

						numSkippedWhite = 0;
						boolean bIsSB = m_TokenList.Add(curToken, m_SBR,
								m_SBRModel);
					...
					// Lets buffer trailing punctuations (if different from
					// leading)
					alTrailBuffer.clear();
					int nNumTrail = 0;
					...
					if (nNumLead < nStringLength) {
						...
						while (!Character.isLetterOrDigit(strCurrentDecomposed
								.charAt(nStringLength - nNumTrail - 1))) {
							// Handle Unbreakables
							if (m_UnbreakableLookup
									.IsUnbreakable(strCurrentDecomposed
											.substring(nNumLead, nStringLength
													- nNumTrail))) {
								break;
							}
							strCurrentRawTok = strCurrentDecomposed.substring(
									nStringLength - nNumTrail - 1,
									nStringLength - nNumTrail);
							strCurrentCanonTok = Canonizer.CanonForm(
									strCurrentRawTok,
									Canonizer.RULE_NORMALIZE_PUNCT,
									m_PunctLookup, m_LigatureLookup,
									m_AccentLookup);
							alTrailBuffer.add(new String[] { strCurrentRawTok,
									strCurrentCanonTok });
							++nNumTrail;
						}
					}
					...
					if (nNumLead < nStringLength) {
						strCurrentRawTok = strCurrentDecomposed.substring(
								nNumLead, nStringLength - nNumTrail);

						if (!strCurrentRawTok.equals("")) {
							int nTokenType = TokenConsts.TYPE_WORD;
							if (strCurrentRawTok.length() == 1
									&& !Character
											.isLetterOrDigit(strCurrentRawTok
													.charAt(0))) {
								...
								strCurrentCanonTok = Canonizer.CanonForm(
										strCurrentRawTok, m_Rules,
										m_PunctLookup, m_LigatureLookup,
										m_AccentLookup);
							}

							Token curToken = new Token(strCurrentRawTok,
									strCurrentCanonTok, nTokenType,
									m_PunctLookup, m_AccentLookup,
									m_NumTokens++, m_NumSentences,
									numSkippedWhite, m_NumChars,
									m_NETypes.length);
							m_NumChars = curToken.EndPos();
					...
					ListIterator<String[]> iTrailCur = alTrailBuffer
							.listIterator(alTrailBuffer.size());
					while (iTrailCur.hasPrevious()) {
						String[] strBuffered = (String[]) iTrailCur.previous();
						Token curToken = new Token(strBuffered[0],
								strBuffered[1], TokenConsts.TYPE_PUNCTUATION,
								m_PunctLookup, m_AccentLookup, m_NumTokens++,
								m_NumSentences, 0, m_NumChars, m_NETypes.length);
						m_NumChars = curToken.EndPos();
						boolean bIsSB = m_TokenList.Add(curToken, m_SBR,
								m_SBRModel);
						if (bIsSB) {
		...
		// Close last sentence..
		++m_NumSentences;
		// TODO: may happend that last sentence is not recognized as "all cap"
		// if the text is not ending with \n that helps detecting last sentence
		if (m_NumTokens != m_TokenList.Size()) {
			throw new Error(
					"Inconsistant TokenList. Num tokens counted by tokenizer and tokenlist mismatch.");
		}
	}
	...
	/**
	 * Set the rules the {@link Canonizer} will operates with. The default value
	 * is set at construction time and is something like:
	 * 
	...
	/**
	 * Reset the Tokenizer in order to process another text. Can be called on a
	 * tokenizer even if no text has been processed.
	 */
	public void Reset() {
		m_NumTokens = 0;
		m_NumSentences = 0;
	...
	/**
	 * Reset the Tokenizer in order to process another text, in a different
	 * language. Can be called on a tokenizer even if no text has been
	 * processed.
	 * 
	 * @param pi_Language
	 *            New language the tokenizer will accept
	 */
