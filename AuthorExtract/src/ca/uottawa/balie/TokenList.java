...
/**
 * List of Tokens to represent a text. Comes with a bunch of manipulation
 * functions. Also an XML representation.
 * 
...
public class TokenList implements Serializable {
	 ...
	 *            True if the sentences boundaries must be detected
	 */
	 ...
	 */
	public boolean Add(Token pi_Token, SentenceBoundariesRecognition pi_SBR,
		...
		if (TokenConsts.Is(pi_Token.Type(), TokenConsts.TYPE_WORD)) {
			if (m_TermFrequency.containsKey(pi_Token.Canon())) {
				double nFreq = ((Double) m_TermFrequency.get(pi_Token.Canon()))
						.doubleValue();
		...
		if (m_DetectSentenceBoundaries) {
			...
			if (bIsSB) {
				m_SentenceTokenBounds.add(new Integer(
						(int) pi_Token.Position() - 1));
			}
		}
	...
	private boolean CheckForSentenceBoundary(
			SentenceBoundariesRecognition pi_SBR, WekaLearner pi_SBRModel) {
		boolean bIsSB = false;
		...
		if (m_NumToken > 1) {
			if (m_NumToken == 2) {
				...
				m_tokNext = (Token) m_HashAccess
						.get(new Integer(m_NumToken - 1));
			...
			if (pi_SBR.IsSentenceBoundary(pi_SBRModel, m_tokSentBeginning,
					m_tokLast, m_tokCurrent, m_tokNext)) {
	 ...
	 * 
	 * @param pi_Index
	 *            Index of the sentence to get (in number of sentences)
	 * @param pi_Canonic
	 *            True if the text must be returned in its canonical version
	 * @param pi_PrintNewLines
	 *            Print \n characters
	 * @return The text of a sentence (String)
	 */
	public String SentenceText(int pi_Index, boolean pi_Canonic,
			boolean pi_PrintNewLines) {

		int maxSentenceTokenPos = (pi_Index == m_SentenceTokenBounds.size()) ? m_NumToken
				: ((Integer) m_SentenceTokenBounds.get(pi_Index)).intValue() + 1;
		int minSentenceTokenPos = pi_Index == 0 ? 0
				: ((Integer) m_SentenceTokenBounds.get(pi_Index - 1))
						.intValue() + 1;

		return TokenRangeText(minSentenceTokenPos, maxSentenceTokenPos,
				pi_Canonic, pi_PrintNewLines, false, false, false, false);
	}
	 ...
	 * 
	 * @param pi_Start
	 *            Start token number (inclusive)
	 * @param pi_Stop
	 *            end token number (exclusive)
	 * @param pi_Canonic
	 *            print in canonical (lowercased, etc) form
	 * @param pi_PrintNewLines
	 *            print \n characters
	 * @param pi_TagEntities
	 *            add XML tags around named entities
	 * @param pi_AddAlias
	 *            add alias network infos in XML tag
	 * @param pi_AddExplanation
	 *            add explanations infos in XML tag
	 * @param pi_EscapeXML
	 *            escape XML reserved characters in the text (so that output is
	 *            valid XML)
	 * 
	 * @return textual representation of the tokenlist
	 */
	public String TokenRangeText(int pi_Start, int pi_Stop, boolean pi_Canonic,
			boolean pi_PrintNewLines, boolean pi_TagEntities,
			boolean pi_AddAlias, boolean pi_AddExplanation, boolean pi_EscapeXML) {
		...
		for (int i = pi_Start; i != pi_Stop; ++i) {
			...
			// TODO Sahar
			if (pi_PrintNewLines
					|| (tokCur.Type() != TokenConsts.TYPE_PUNCTUATION)) { // ||
				// tokCur
				// .
				// PartOfSpeech
				// (
				...
				// open entity tag
				if (pi_TagEntities && tokCur.EntityType().IsStart()
						&& tokCur.EntityType().GetLabel(m_NETypes) != null) {
					...
					if (pi_AddExplanation) {
						strTok.append("\" INFO=\"");
						strTok.append(XmlUtil.Encode(tokCur.EntityType()
								.GetInfo()));
					}
					strTok.append("\">");
				}
				...
				if (pi_EscapeXML) {
					strTok.append(XmlUtil.Encode(pi_Canonic ? tokCur.Canon()
							: tokCur.Raw()));
				} else {
				...
				// close entity tag
				if (pi_TagEntities && tokCur.EntityType().IsEnd()
						&& tokCur.EntityType().GetLabel(m_NETypes) != null) {
	...
	public int getSentenceCount() {
		return (m_NumToken - 1 > 0) ? Get(m_NumToken - 1).SentenceNumber() + 1
				: 0;
	}

	/**
	 * Gets the TF table. That is a lookup that maps words to their frequency in
	 * the text.
	 * 
	 * @return Hashtable
	 */
	 ...
	 * 
	 * @param pi_Index
	 *            Index of the token to update
	 * @param pi_POS
	 *            Part-of-speech of this token (see {@link TokenConsts} for the
	 *            enumeration)
	 * @see TokenConsts
	 */
	public void SetPOS(int pi_Index, String pi_POS) {
	 ...
	 *            index of the entity
	 * @param pi_Match
	 *            string that matches in lexicon
	 */
