...
public class LanguageIdentification {
	...
	/**
	 * The latest LangID model (if found) is loaded upon construction. Language
	 * guesser will only work if a model is found.
	 */
	public LanguageIdentification() {
		...
		try {
			...
			if (m_Model == null) {
				throw new Error(
						"Unable to find the language identification model :"
								+ Balie.LANGUAGE_ID_MODEL);
			}

		} catch (Exception e) {
			if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
				DebugInfo.Out("No LanguageIdentification model found.");
		}
	}
	...
	private WekaLearner TrainModel(ArrayList<String> pi_Languages) {
		...
		if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
			DebugInfo.Out("Reading files.");

		while (iCur.hasNext()) {
			String strCurLan = (String) iCur.next();
			if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
				DebugInfo.Out(strCurLan);
			ArrayList<CharacterNGram> alCurNGrams = null;
			try {
				alCurNGrams = Files2NGram(Balie.LANGUAGE_ID_TRAINING_CORPUS,
						strCurLan, hashAllNGrams, hashAllUNIGrams);
			} catch (Exception e) {
				throw new Error("Training corpus was not found here: "
						+ Balie.LANGUAGE_ID_TRAINING_CORPUS);
			}
		...
		if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
			DebugInfo.Out("Attribute Selection...");
		WekaAttribute[] wekaAttr = new WekaAttribute[alSelectedNgramAttributes
				.size()];
		for (int i = 0; i != alSelectedNgramAttributes.size(); ++i) {
			wekaAttr[i] = new WekaAttribute((String) alSelectedNgramAttributes
					.get(i));
		}

		// String[] strAttr = (String[])alSelectedNgramAttributes.toArray(new
		// String [alSelectedNgramAttributes.size()]);
		String[] strClass = (String[]) pi_Languages
				.toArray(new String[pi_Languages.size()]);

		WekaAttributeSelection was = new WekaAttributeSelection(
				WekaAttributeSelection.WEKA_CHI_SQUARE, wekaAttr, strClass);
		...
		while (iCur.hasNext()) {
			...
			while (iNCur.hasNext()) {
				...
				Double[] nGram = cng
						.Instance((String[]) alSelectedNgramAttributes
								.toArray(new String[0]));
				was.AddInstance(nGram, strCurLan);
			}
			if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
				DebugInfo.Out(String.valueOf(alCurNGrams.size())
						+ " instances created for " + strCurLan);
		}
		...
		String[] strAllAttributes = new String[m_NGramMaxNum
				+ alSelectedUnigramAttributes.size()];
		WekaAttribute[] wekaFinalAttr = new WekaAttribute[strAllAttributes.length];

		if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
			DebugInfo.Out("Creating the classifier...");
		...
		// At the list of the Reduced Attributes(NGrams) add the list of
		// UNIGrams
		int nPos = strReducedAttr.length;
		for (int i = 0; i != alSelectedUnigramAttributes.size(); ++i) {
			wekaFinalAttr[nPos] = new WekaAttribute(
					(String) alSelectedUnigramAttributes.get(i));
		...
		while (iCur.hasNext()) {
			...
			if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
				DebugInfo.Out(String.valueOf(alCurNGrams.size())
						+ " TRAIN instances created for " + strCurLan);
		}
		...
		/*
		 * // Create a classifier if (MODE.equals(Balie.LANGUAGE_ID_MODEL_HUGE))
		 * { SMO smo = new SMO(); wl.CreateModel(smo); } else { NaiveBayes nb =
		 * new NaiveBayes(); //nb.setUseKernelEstimator(true);
		 * wl.CreateModel(nb); }
		 */
		return wl;
	}
	...
	private void TestModel(WekaLearner pi_Model, ArrayList<String> pi_Languages) {
		...
		while (iCur.hasNext()) {
			...
			ArrayList<CharacterNGram> alCurNGrams = null;
			try {
				alCurNGrams = Files2NGram(Balie.LANGUAGE_ID_TESTING_CORPUS,
						strCurLan, hashAllNGrams, hashAllUNIGrams);
			} catch (Exception e) {
				throw new Error("Testing corpus was not found here: "
						+ Balie.LANGUAGE_ID_TESTING_CORPUS);
			}
		...
		while (iCur.hasNext()) {
			...
			if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
				DebugInfo.Out(String.valueOf(alCurNGrams.size())
						+ " TEST instances created for " + strCurLan);
		}

		System.out.println(pi_Model.TestModel());
	}

	private ArrayList<String> GetGlobalNGramList(
			Hashtable<String, Integer> pi_hashAllNGrams) {
		...
		if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
			DebugInfo.Out(String.valueOf(alSelectedAttributes.size())
					+ " attributes found");
		return alSelectedAttributes;

	}

	private ArrayList<CharacterNGram> Files2NGram(String pi_Corpus,
			String pi_Language, Hashtable<String, Integer> pi_hashAllNGrams,
			Hashtable<String, Integer> pi_hashAllUNIGrams) {
				...
				try {
					...
					while (N_keyEnum.hasMoreElements()) {
						...
						if (pi_hashAllNGrams.containsKey(strNGram)) {
							int old = ((Integer) pi_hashAllNGrams.get(strNGram))
									.intValue();
							int cur = ((Integer) NGR_freq.get(strNGram))
									.intValue();
							pi_hashAllNGrams.put(strNGram, new Integer(old
									+ cur));
						} else {
							pi_hashAllNGrams.put(strNGram, NGR_freq
									.get(strNGram));
						}
					}
					...
					while (UNI_keyEnum.hasMoreElements()) {
						...
						if (pi_hashAllUNIGrams.containsKey(strUNIGram)) {
							int old = ((Integer) pi_hashAllUNIGrams
									.get(strUNIGram)).intValue();
							int cur = ((Integer) UNIGR_freq.get(strUNIGram))
									.intValue();
							pi_hashAllUNIGrams.put(strUNIGram, new Integer(old
									+ cur));
						} else {
							pi_hashAllUNIGrams.put(strUNIGram, UNIGR_freq
									.get(strUNIGram));
						}
					}
	...
	public LanguageIdentificationGuess[] GetLanguageLikelihood(String pi_Text) {
		...
		if (strLangs.length != fProb.length) {
			throw new Error(
					"Incompatible language list and probability distribution.");
		}

		LanguageIdentificationGuess guesses[] = new LanguageIdentificationGuess[strLangs.length];

		for (int i = 0; i != strLangs.length; ++i) {
			guesses[i] = new LanguageIdentificationGuess(strLangs[i],
					new Double(fProb[i]));
			if (Balie.DEBUG_LANGUAGE_IDENTIFICATION)
				DebugInfo.Out(strLangs[i] + "(" + fProb[i] + ")");
		}
	 ...
	 * @param pi_Text
	 *            A text for which we want to guess the language
	 * @return String that represent the most probable language (see
	 *         {@link Balie} for enumeration)
	 * @see Balie
	 */
	public String DetectLanguage(String pi_Text) {
		...
		// TODO: implements better heuristic in case many language are
		// probable..
		strLanguage = guesses[0].Language();
		System.out.println(guesses[0].Language() + " - "
				+ guesses[0].Probability());

		System.out.println(guesses[1].Language() + " - "
	...
	public static void TrainLanguageIdentification() {
		...
		WekaPersistance.PrintToArffFile(wl, Balie.OUT_LI_TRAIN_MODEL,
				WekaPersistance.PRINT_TRAINING_SET);
		WekaPersistance.PrintToArffFile(wl, Balie.OUT_LI_TEST_MODEL,
