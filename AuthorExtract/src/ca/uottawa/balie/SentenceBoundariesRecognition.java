...
/**
 * Methods for training, testing and using sentence boundary recognition. This
 * class needs to be redesigned to improve usability. In its current form, it
 * requires the following manipulations to be used correctly.
 * <ul>
 * <li>First, it operates on a TokenList</li>
 * <li>The caller module must obtain the model: <code>GetModel()</code></li>
 * <li>It must operated by examining every token and keeping a look ahead of 1
 * token</li>
 * <li>The method <code>IsSentenceBoundary()</code> must be called on each token
 * </li>
 * <li>When a sentence break is found, the method
 * <code>YieldSentenceBeginning()</code> must be called.</li>
 * </ul>
 * 
...
public class SentenceBoundariesRecognition {
	...
	private WekaLearner TrainModel() {
		...
		String[] strClass = new String[] { IS_SENTENCE_BOUNDARY,
				IS_NOT_SENTENCE_BOUNDARY };
		...
		for (int i = 0; i != alTokenLists.size(); ++i) {
			...
			for (int j = 0; j != alCurrentTokenList.Size(); ++j) {
				...
				boolean bTrivialInstance = DescribeTrainTestInstance(
						alCurrentTokenList, alNextTokenList, j, strInstance);
	...
	private boolean DescribeTrainTestInstance(TokenList pi_CurTokenList,
			TokenList pi_NextTokenList, int pi_Pos, Object[] pi_Instance) {
		...
		if (pi_CurTokenList.Get(pi_Pos).NumWhiteBefore() > 0
				|| (TokenConsts.Is(pi_CurTokenList.Get(pi_Pos).Type(),
						TokenConsts.TYPE_PUNCTUATION) && TokenConsts.Is(
						pi_CurTokenList.Get(pi_Pos).PartOfSpeech().hashCode(),
						TokenConsts.PUNCT_NEWLINE))) {
		...
		if (pi_Pos != pi_CurTokenList.Size() - 1) {
			...
			if (pi_CurTokenList.Get(pi_Pos + 1).NumWhiteBefore() > 0
					|| (TokenConsts.Is(pi_CurTokenList.Get(pi_Pos + 1).Type(),
							TokenConsts.TYPE_PUNCTUATION) && TokenConsts.Is(
							pi_CurTokenList.Get(pi_Pos + 1).PartOfSpeech()
									.hashCode(), TokenConsts.PUNCT_NEWLINE))) {
			...
			if (pi_NextTokenList.Get(0).NumWhiteBefore() > 0
					|| (TokenConsts.Is(pi_CurTokenList.Get(0).Type(),
							TokenConsts.TYPE_PUNCTUATION) && TokenConsts.Is(
							pi_CurTokenList.Get(0).PartOfSpeech().hashCode(),
							TokenConsts.PUNCT_NEWLINE))) {
				pi_Instance[4] = new Double(1);
			}
		}
	...
	private static boolean IsTrivialBoundary(Object[] pi_Instance) {
		...
		// These are cases where all attributes are lowercased words and case
		// where first token would be a weird punctuation
		if (pi_Instance.length != NUM_FEATURES) {
			throw new Error("Invalid instance");
		}
		return pi_Instance[5].equals(VAL_PUNCT)
				|| (IsTrivialValue((String) pi_Instance[0], true)
						&& IsTrivialValue((String) pi_Instance[1], false)
						&& IsTrivialValue((String) pi_Instance[3], false) && IsTrivialValue(
						(String) pi_Instance[5], false));
	}
	...
	private String GetTokenValue(Token pi_Token) {
		...
		if (pi_Token == null) {
			ret = VAL_NULL;
				...
				.Is(pi_Token.Type(), TokenConsts.TYPE_PUNCTUATION)) {
			if (TokenConsts.Is(pi_Token.PartOfSpeech().hashCode(),
					TokenConsts.PUNCT_PERIOD)) {
				ret = VAL_PERIOD;
			} else if (TokenConsts.Is(pi_Token.PartOfSpeech().hashCode(),
					TokenConsts.PUNCT_EXCLAMATION)
					|| TokenConsts.Is(pi_Token.PartOfSpeech().hashCode(),
							TokenConsts.PUNCT_INTERROGATION)) {
				ret = VAL_PERIOD_LIKE;
			} else if (TokenConsts.Is(pi_Token.PartOfSpeech().hashCode(),
					TokenConsts.PUNCT_NEWLINE)) {
				...
				if (m_bCurrentSentenceIsAllCap) {
					...
					TokenConsts.PUNCT_OPEN_PARENTHESIS)
					|| TokenConsts.Is(pi_Token.PartOfSpeech().hashCode(),
							TokenConsts.PUNCT_OPEN_BRACKET)) {
				ret = VAL_OPEN_BRACKET;
			} else if (TokenConsts.Is(pi_Token.PartOfSpeech().hashCode(),
					TokenConsts.PUNCT_CLOSE_PARENTHESIS)
					|| TokenConsts.Is(pi_Token.PartOfSpeech().hashCode(),
							TokenConsts.PUNCT_CLOSE_BRACKET)) {
				ret = VAL_CLOSE_BRACKET;
			} else if (TokenConsts.Is(pi_Token.PartOfSpeech().hashCode(),
					TokenConsts.PUNCT_QUOTE)) {
				ret = VAL_QUOTE;
			} else {
				ret = VAL_PUNCT;
			}
		...
		} else if (TokenFeature.Feature.StartsWithCapital.Mechanism()
				.GetBooleanValue(pi_Token.Features())) {
			ret = VAL_CAPITAL;
			...
			// Sentence is not more an "All capitalized sentence" if such a
			// word, with suffisent length, is found
	...
	private static ArrayList<TokenList> GetTokenLists(
			ArrayList<String> pi_Sentences) {
	...
	/**
	 * Check if a given token sequence is a sentence break (located after the
	 * "current" token)
	 * 
	 * @param pi_Model
	 *            SBD model (use the GetModel() method)
	 * @param pi_SentenceBeginning
	 *            The first token of the sentence (use the first token of the
	 *            text for the first sentence)
	 * @param pi_LastToken
	 *            The previous token (i-1)
	 * @param pi_CurrentToken
	 *            The current token under examination (i)
	 * @param pi_NextToken
	...
	public boolean IsSentenceBoundary(WekaLearner pi_Model,
			Token pi_SentenceBeginning, Token pi_LastToken,
		...
		if (pi_CurrentToken != null
				&& pi_CurrentToken.NumWhiteBefore() > 0
				|| (TokenConsts.Is(pi_CurrentToken.Type(),
						TokenConsts.TYPE_PUNCTUATION) && TokenConsts.Is(
						pi_CurrentToken.PartOfSpeech().hashCode(),
						TokenConsts.PUNCT_NEWLINE))) {
		...
		if (pi_NextToken != null
				&& pi_NextToken.NumWhiteBefore() > 0
				|| (TokenConsts.Is(pi_NextToken.Type(),
						TokenConsts.TYPE_PUNCTUATION) && TokenConsts.Is(
						pi_NextToken.PartOfSpeech().hashCode(),
						TokenConsts.PUNCT_NEWLINE))) {
		...
		if (!IsTrivialBoundary(strInstance)) {
			String strBoundary = ((String[]) pi_Model.GetClassList())[(int) pi_Model
					.Classify(strInstance)];
	...
	public static void TrainSentenceBoundariesRecognition() {
		...
		SentenceBoundariesRecognition sbr = new SentenceBoundariesRecognition(
				new LanguageSpecificEnglish());
		WekaLearner wl = sbr.TrainModel();
		sbr.TestModel(wl);

		// output arff files for visualization & debugging
		WekaPersistance.PrintToArffFile(wl, Balie.OUT_SBD_TRAIN_MODEL,
				WekaPersistance.PRINT_TRAINING_SET);
		WekaPersistance.PrintToArffFile(wl, Balie.OUT_SBD_TEST_MODEL,
				WekaPersistance.PRINT_TESTING_SET);
		...
		if (Balie.DEBUG_PRINT_SBD_TEST_CORPUS) {
			try {
				String strContent = FileHandler.GetTextFileContent(
						Balie.SBR_TESTING_CORPUS_PC, Balie.ENCODING_UTF8);
				strContent = strContent.replaceAll("<[^>]*>", "");
				LanguageIdentification li = new LanguageIdentification();
				Tokenizer tTest = new Tokenizer(li.DetectLanguage(strContent),
						true);
				...
				for (int i = 0; i != tTest.SentenceCount(); ++i) {
					System.out.print("(" + i + ")");
					System.out
							.println(alTokenList.SentenceText(i, false, true));
				}
			} catch (Exception e) {
