...
/**
 * Tokens are the unit element of Balie. A text is represneted as a list of
 * consecutives tokens (called {@link TokenList}).
 * <ul>
 * <li>For punctuation token, the POS is resolved at construction time.</li>
 * <li>For word token, the POS must be resolved by examining the whole
 * tokenlist.</li>
 * </ul>
 * 
...
public class Token implements Serializable {
	 ...
	 * @param pi_CanonLiteral
	 *            The canonical version of the word
	 * @param pi_Type
	 *            The type (punctuation or word) see {@link TokenConsts} for
	 *            details
	 * @param pi_PunctLookup
	 *            The lookup table for punctuation types
	 * @param pi_Position
	 *            The position of the token, in number of words from the text
	 *            beginning
	 * @param pi_Sentence
	 *            The sentence number
	 * @param pi_NumWhiteBefore
	 *            Number fo white chars prior to this token
	 * @param pi_NextStart
	 *            Start position (in chars) of this token (including white)
	 */
	public Token(String pi_RawLiteral, String pi_CanonLiteral, int pi_Type,
			PunctLookup pi_PunctLookup, AccentLookup pi_AccentLookup,
			int pi_Position, int pi_Sentence, int pi_NumWhiteBefore,
			int pi_NextStart, int pi_NETagSetSize) {
		...
		if (TokenConsts.Is(m_Type, TokenConsts.TYPE_WORD)
				|| TokenConsts.Is(m_Type, TokenConsts.TYPE_PUNCTUATION)) {
			// TODO par Sahar
			String pos = POS(pi_RawLiteral.trim());
			m_PartOfSpeech = pos;
			// m_PartOfSpeech = TokenConsts.POS_UNKNOWN; // must be resolved by
			// an external product
			// (e.g., qTag)
	...
	// TODO par Sahar
	String POS(String word) {
		String tag = "";
		String tab[] = { word };

		Tagger tagger = Tokenizer.tagger;
		String[] tags = tagger.tag(tab);
		tag = tags[0];

		// System.out.println(" TAGG -- " + word + " : " + tag);
		return tag;
	}
	...
	/**
	 * Gets the type of the token (word or punctuation). see {@link TokenConsts}
	 * for enumeration.
	 * 
	...
	/**
	 * Gets the part-of-speech of the token. Words and punctuations have a POS.
	...
	/**
	 * Set the part-of-speech of the token. Words and punctuations have a POS.
	 * see {@link TokenConsts} for enumeration of both.
	 * 
	 * @param pi_POS
	 *            the pos
	 * @see TokenConsts
	 */
	protected void PartOfSpeech(String pi_POS) {
	...
	/**
	 * Get the entity type of this token see {@link NamedEntityType} for
	...
	/**
	 * Set the entity type see {@link TokenConsts} for enumeration of types.
	 * 
	 * @param pi_Type
	 * @see TokenConsts
	 */
	...
	/**
	 * Increments the sentence number of a token. Useful if using the SBR module
	 * that can identify sentence break on the late.
	 */
	...
	public StringBuffer ToXML() {
		...
		sb.append("<token type=\"" + m_Type + "\" entity-type=\""
				+ m_EntityType + "\" canon=\"" + XmlUtil.Encode(m_CanonLiteral)
				+ "\">");
	...
	/**
	 * @return the start position in number of character relative to document
	 *         start
	 */
	public int StartPos() {
		return m_StartPos;
	...
	/**
	 * @return the end position in number of character relative to document
	 *         start
	 */
	public int EndPos() {
		return m_EndPos;
	...
	public String toString() {
		return m_CanonLiteral.toString() + ":Pos=" + m_Position + ":Sent="
				+ m_Sentence;
	}
	 ...
	 * 
	 * @param pi_Match
	 *            lexicon entry matching the entity starting on this token
	 */
