...
public class NamedEntityRecognitionNerf extends NamedEntityRecognition {
	...
	public NamedEntityRecognitionNerf(TokenList pi_TokenList,
			LexiconOnDiskI pi_Lexicon, DisambiguationRulesI pi_Rules,
			PriorCorrectionI pi_Prior, NamedEntityTypeEnumI[] pi_Mapping,
	...
	private void CheckEntityNounAmbiguityNerf() {
		...
		for (int i = 0; i != m_TokenList.Size(); ++i) {
			...
			if (bLastTokenIsQuote || tok.IsSentenceStart()
					|| bInsideAllCapitalized) {
				if (TokenFeature.Feature.StartsWithCapital.Mechanism()
						.GetBooleanValue(tok.Features())
						&& TokenConsts.Is(tok.Type(), TokenConsts.TYPE_WORD)) {
					NERServices.AddIndexExt(hAmbiguous,
							tok.Raw().toLowerCase(), i);
				}
			}
			...
			if (TokenConsts.Is(tok.Type(), TokenConsts.TYPE_WORD)) {
				if (TokenFeature.Feature.StartsWithCapital.Mechanism()
						.GetBooleanValue(tok.Features())) {
					NERServices.AddIndexExt(m_CapWord2Index, tok.Raw()
							.toLowerCase(), i);
				} else {
					NERServices.AddIndexExt(hLowercased, tok.Raw()
							.toLowerCase(), i);
				}
			}
			...
			if (TokenConsts.Is(tok.Type(), TokenConsts.TYPE_PUNCTUATION)
					&& (TokenConsts.Is(tok.PartOfSpeech().hashCode(),
							TokenConsts.PUNCT_QUOTE) || TokenConsts.Is(tok
							.PartOfSpeech().hashCode(), TokenConsts.PUNCT_APOSTROPHE))) {
		...
		while (eCur.hasMoreElements()) {
			...
			if (hLowercased.containsKey(strCur)) {
				alRejectedBothCases.addAll((ArrayList<Integer>) m_CapWord2Index
						.get(strCur));
				alRejectedBothCases.addAll((ArrayList<Integer>) hLowercased
						.get(strCur));
			}

			// invalidate word that appears only in ambiguous pos
			else if (hAmbiguous.containsKey(strCur)) {
				ArrayList<Integer> alCap = (ArrayList<Integer>) m_CapWord2Index
						.get(strCur);
				ArrayList<Integer> alAmb = (ArrayList<Integer>) hAmbiguous
						.get(strCur);
		...
		while (enumL.hasMoreElements()) {
			...
			if (!m_CapWord2Index.containsKey(strCur)) {
				alRejectedAlwaysLower.addAll((ArrayList<Integer>) hLowercased
						.get(strCur));
			}
		}
				...
				ForceEntityTypeExt(
						...
						NamedEntityType
								.Intersection(
										net,
										NamedEntityTypeEnum
												.NaturallyOccursLowercased(),
										new NamedEntityExplanation(
												NamedEntityExplanation.ExplanationType.NATURAL_LOWERCSE,
												new Object[] {})));
			} else if (NERServices.OneWordEntity(net)) {
				ForceEntityTypeExt(
						nIdx,
						new NamedEntityType(
								NamedEntityTypeEnum.NOTHING,
								m_TagSetSize,
								new NamedEntityExplanation(
		...
		for (int i = 0; i != m_TokenList.Size() - 1; ++i) {
			...
			if (curTok.SentenceNumber() == nextTok.SentenceNumber()
				...
				if (nextTok.EntityType().IsStart()) {
					if (m_FullDebug)
						System.out.print("Adjacent entities: " + curTok.Raw()
								+ " | " + nextTok.Raw());
					if (curTok.EntityType().GetLabel(m_TokenList.NETagSet()) != null
							&& nextTok.EntityType().GetLabel(
									m_TokenList.NETagSet()) != null) {

						Hashtable<String, NamedEntityTypeEnumI> hSharedLabs = NERServices
								.SharedLabel(curTok.EntityType(), nextTok
										.EntityType(), m_TokenList.NETagSet());

						if (hSharedLabs.size() > 0) {
							NamedEntityType forceType = NERServices
									.Labels2Types(hSharedLabs, m_TagSetSize);

							if (forceType.IsStart()) {
								forceType.RemoveType(NamedEntityTypeEnum.START,
										null);
							}
							if (forceType.IsEnd()) {
								forceType.RemoveType(NamedEntityTypeEnum.END,
										null);
							}

							if (forceType.Intersect(NamedEntityTypeEnum
									.Mergable())) {

								// bring nextTok explanations to curTok
								curTok.EntityType().AddTrailingInfo(
										nextTok.EntityType().GetInfo());
				...
				// a period or a ampersand follows.
						...
						&& TokenConsts.Is(nextTok.Type(),
								TokenConsts.TYPE_PUNCTUATION)
						&& (TokenConsts.Is(nextTok.PartOfSpeech().hashCode(),
								TokenConsts.PUNCT_PERIOD) || TokenConsts.Is(
								nextTok.PartOfSpeech().hashCode(),
								TokenConsts.PUNCT_AMPERSAND))) {
					Token secondNextTok = m_TokenList.Get(i + 2);
					if (secondNextTok.EntityType().IsStart()
							&& curTok.SentenceNumber() == secondNextTok
									.SentenceNumber()) {
						if (m_FullDebug)
							System.out.print("Adjacent entities: "
									+ curTok.Raw() + " | " + nextTok.Raw()
									+ " | " + secondNextTok.Raw());

						Hashtable<String, NamedEntityTypeEnumI> hSharedLabs = NERServices
								.SharedLabel(curTok.EntityType(), secondNextTok
										.EntityType(), m_TokenList.NETagSet());
						...
						if (hSharedLabs.size() > 0) {
							...
							if (forceType.Intersect(NamedEntityTypeEnum
								...
								// bring nextTok explanations to curTok
								curTok.EntityType().AddTrailingInfo(
										secondNextTok.EntityType().GetInfo());
								MergeEntitiesExt(i, curTok, secondNextTok,
										forceType);
								if (m_FullDebug)
									System.out.println(" --> Merged!");
							} else {
	...
	private void CheckEntityEntityAmbiguityNerf() {
		...
		NamedEntityAliasResolution near = new NamedEntityAliasResolution(
				m_TokenList, m_TagSetSize, m_DesiredEntities);
		...
		for (int i = 0; i != m_TokenList.Size() - 1; ++i) {
			...
			if (NamedEntityAliasResolution.ValidAliasToken(curTok)) {
				NamedEntityType curType = NamedEntityType.Intersection(curTok
						.EntityType(), m_DesiredEntities, null);

				// check all aliases for a cue
				Integer nGroupId = curTok.NamedEntityAlias();
				NamedEntityAlias neAlias = near.GetAlias(nGroupId);

				if (!neAlias.DominantType().HasNoTag()
						&& !neAlias.DominantType().equals(curType)) {
					NamedEntityType net = neAlias
							.DominantType()
							.clone(
									new NamedEntityExplanation(
					...
					if (curTok.EntityType().HasNoTag()) {
						...
						if (m_FullDebug)
							System.out.println("Force type: "
									+ curTok.Raw()
									+ " --> "
									+ neAlias.DominantType().GetLabel(
											m_TokenList.NETagSet()));
					} else {
	...
	private void ApplyEntityEntityClassifiersNerf() {
		...
		for (int i = 0; i != m_TokenList.Size() - 1; ++i) {
			...
			if (curTok.EntityType().IsStart()) {
				NamedEntityType curTypes = NamedEntityType.Intersection(curTok
						.EntityType(), m_DesiredEntities, null);
				...
				if (curTypes.TypeCount() > 1) {
					...
					// right
					for (int k = nEnd + 1; k != nEnd + 1
							+ m_Rules.ContextSize(); ++k) {
					...
					if (netBefore != null && curTypes.Intersect(netBefore)) {
						ForceEntityTypeExt(
								...
								NamedEntityType
										.Intersection(
												...
												netBefore,
												new NamedEntityExplanation(
														NamedEntityExplanation.ExplanationType.ADJACENT_SHARE_TYPE,
														new Object[] {})));
						if (m_FullDebug)
							System.out
									.println("--> merged with preceeding of same type");
						...
						if (m_FullDebug)
							System.out
									.println("--> merged with following of same type");
					} else {
						// classify!
						...
						if (hLabel2NEType.containsKey("month")) {
							...
							ForceEntityTypeExt(
									i,
									new NamedEntityType(
											hLabel2NEType.get("month"),
											NETagSet.length,
											new NamedEntityExplanation(
													NamedEntityExplanation.ExplanationType.CLASSIFIER,
													new Object[] {
															curTok.Raw(),
															hLabel2NEType.get(
																	"month")
							...
							if (m_FullDebug)
								System.out
										.println("--> classified as: month (special rule)");
						}

						else if (curTypes.TypeCount() == 2) {

							// get prior for this entity
							double[] fPrior = m_Prior.GetPrior(curTok
									.LexiconMatch(), strTypes[0], strTypes[1]);
							...
							DisambiguationRulesKey key = new DisambiguationRulesKey(
									strTypes[0], strTypes[1]);
							DisambiguationRulesOutcome outcome = m_Rules
									.Classify(key, strCtx, hPriorMap);
							if (m_FullDebug)
							...
							ForceEntityTypeExt(
									i,
									new NamedEntityType(
											hLabel2NEType.get(outcome.Class()),
											NETagSet.length,
											new NamedEntityExplanation(
													NamedEntityExplanation.ExplanationType.CLASSIFIER,
													new Object[] {
							...
							while (cg.hasMore()) {
								...
								double[] fPrior = m_Prior.GetPrior(curTok
										.LexiconMatch(),
										strTypes[classPair[0]],
										strTypes[classPair[1]]);
								hPriorMap.put(strTypes[classPair[0]],
										new Double(fPrior[0]));
								hPriorMap.put(strTypes[classPair[1]],
										new Double(fPrior[1]));
								...
								hVote.put(outcome.Class(), new Double(fCurVote
										.doubleValue()
										+ outcome.Likelihood()));
							}
							...
							double fSum = 0.0f;
							for (int j = 0; j != strTypes.length; ++j) {
								double fCurVote = hVote.get(strTypes[j])
										.doubleValue();
								fSum += fCurVote;
							...
							ForceEntityTypeExt(
									...
									new NamedEntityType(
											hLabel2NEType
													.get(strTypes[nWinner]),
											NETagSet.length,
											new NamedEntityExplanation(
													NamedEntityExplanation.ExplanationType.CLASSIFIER_VOTE,
													new Object[] {
															curTok.Raw(),
															hLabel2NEType
																	.get(
	...
	private void CheckVeryAmbiguousTypes() {
		...
		for (int i = 0; i != m_TokenList.Size(); ++i) {
			...
			if (tok.EntityType().IsStart()
					&& tok.EntityType().Intersect(
							NamedEntityTypeEnum.HighlyAmbiguous())) {
				...
				if (CountCapitalizedWords(i) > 1
						|| (NERServices.OneWordEntity(tok.EntityType()) && (TokenFeature.Feature.IsAllCapitalized
								.Mechanism().GetBooleanValue(tok.Features()) || TokenFeature.Feature.IsMixedCase
								.Mechanism().GetBooleanValue(tok.Features())))) {
					if (m_FullDebug)
				...
				if (nPreceding > 0 && nFollowing < m_TokenList.Size()) {
					...
					if ((TokenFeature.Feature.HasOpenBracketPunct.Mechanism()
							.GetBooleanValue(precTok.Features()) && TokenFeature.Feature.HasCloseBracketPunct
							.Mechanism().GetBooleanValue(nextTok.Features()))
							|| (TokenFeature.Feature.HasOpenParenthesisPunct
									.Mechanism().GetBooleanValue(
											precTok.Features()) && TokenFeature.Feature.HasCloseParenthesisPunct
									.Mechanism().GetBooleanValue(
											nextTok.Features()))) {
						if (m_FullDebug)
							System.out
									.println("Between brackets: " + tok.Raw());
						bDisable = false;
					}
				}
					...
					ForceEntityTypeExt(
							...
							new NamedEntityType(
									...
									new NamedEntityExplanation(
											...
											new Object[] {})));
					if (m_FullDebug)
						System.out.println("Rejected very ambigous: "
								+ tok.Raw());
				}
			}
		}
	...
	private void PostCheckEntityBoundaryNerf() {
		...
		for (int i = 0; i != m_TokenList.Size() - 1; ++i) {
			...
			if (curTok.SentenceNumber() == nextTok.SentenceNumber()
				...
				if (curTok.EntityType().Intersect(
						NamedEntityTypeEnum.Mergable())
						&& !bInsideAllCapitalized
						&& TokenConsts
								.Is(nextTok.Type(), TokenConsts.TYPE_WORD)
						&& nextTok.EntityType().HasNoTag()
						&& TokenFeature.Feature.StartsWithCapital.Mechanism()
								.GetBooleanValue(nextTok.Features())) {
					if (m_FullDebug)
						System.out.println("Entity + capitalized: "
								+ curTok.Raw() + " | " + nextTok.Raw());
					NamedEntityType curType = curTok
							.EntityType()
							.clone(
									new NamedEntityExplanation(
											NamedEntityExplanation.ExplanationType.ENTITY_CAPITAL_MERGING,
											new Object[] { curTok.Raw(),
													nextTok.Raw() }));
				...
				// a period follows any small word (e.g., middle word)
						...
						&& TokenConsts.Is(nextTok.Type(),
								TokenConsts.TYPE_PUNCTUATION)
						&& TokenConsts.Is(nextTok.PartOfSpeech().hashCode(),
								TokenConsts.PUNCT_PERIOD)
						&& curTok.Length() <= 4) {

					// only handle period that is not breaking the sentence
					Token secondNextTok = m_TokenList.Get(i + 2);
					if (curTok.SentenceNumber() == secondNextTok
							.SentenceNumber()) {
						if (m_FullDebug)
				...
				// a hyphen or a ampersand follows an organization name.
						...
						&& TokenConsts.Is(nextTok.Type(),
								TokenConsts.TYPE_PUNCTUATION)
						&& (TokenConsts.Is(nextTok.PartOfSpeech().hashCode(),
								TokenConsts.PUNCT_DASH) || TokenConsts.Is(
								nextTok.PartOfSpeech().hashCode(),
								TokenConsts.PUNCT_AMPERSAND))) {
					...
					if (curTok.EntityType().Intersect(
							...
							&& curTok.SentenceNumber() == secondNextTok
									.SentenceNumber()
							&& TokenFeature.Feature.StartsWithCapital
									.Mechanism().GetBooleanValue(
											secondNextTok.Features())
							&& secondNextTok.EntityType().HasNoTag()) {

						if (m_FullDebug)
							System.out.println("Entity + punct: "
									+ curTok.Raw() + " | " + nextTok.Raw());
						NamedEntityType curType = curTok
								.EntityType()
								.clone(
	...
	private void CheckUnknownCapitalizedExt() {
		...
		while (eCur.hasMoreElements()) {
			...
			// this type
			ArrayList<Integer> alIndexes = (ArrayList<Integer>) m_CapWord2Index
					.get(strUnknown);
			if (alIndexes != null) {
				...
				while (iCur.hasNext()) {
					...
					if (curTok.EntityType().HasNoTag()) {
						...
						if (m_FullDebug)
							System.out.println("Unknown capitalized tagged: "
									+ curTok.Raw()
									+ "("
									+ curTok.EntityType().GetLabel(
											m_TokenList.NETagSet()) + ")");
					}
				}
			}
	...
	private void PromoteUnknownWordToEntity(Token pi_Token,
			NamedEntityType pi_Type) {
		NamedEntityType newNE = new NamedEntityType(m_TagSetSize,
				new NamedEntityExplanation(
						NamedEntityExplanation.ExplanationType.UNKNOWN_CAPITAL,
						new Object[] { pi_Token.Raw() }));
	...
	protected void ExtendEntityExt(int pi_StartIndex, Token pi_FirstBoundary,
			Token pi_NewEnd, NamedEntityType pi_NewType) {
		pi_FirstBoundary.EntityType().RemoveType(NamedEntityTypeEnum.END, null);
		pi_NewEnd.EntityType(new NamedEntityType(NamedEntityTypeEnum.END,
				m_TagSetSize, null));
		ForceEntityTypeExt(pi_StartIndex, pi_NewType);
		if (pi_NewEnd.Raw().length() > 4) {
			m_UnlistedWords.put(pi_NewEnd.Raw().toLowerCase(),
					new UnlistedWordInfo(pi_NewType, pi_FirstBoundary
							.NamedEntityAlias()));
		}
	}

	private void MergeEntitiesExt(int pi_StartIndex, Token pi_FirstBoundary,
			Token pi_SecondBoundary, NamedEntityType pi_NewType) {
		pi_FirstBoundary.EntityType().RemoveType(NamedEntityTypeEnum.END, null);
		pi_SecondBoundary.EntityType().RemoveType(NamedEntityTypeEnum.START,
				null);
		ForceEntityTypeExt(pi_StartIndex, pi_NewType);
	}

	private void ForceEntityTypeExt(int pi_CurPosition,
			NamedEntityType pi_EntityType) {
		...
		if (!curTok.EntityType().IsStart()) {
			...
			while (nPrev > -1) {
				...
				if (bIsStart) {
					prevTok.EntityType().AddType(NamedEntityTypeEnum.START,
							null);
					break;
				}
				--nPrev;
	...
	private int CountCapitalizedWords(int pi_Start) {
		...
		if (TokenFeature.Feature.StartsWithCapital.Mechanism().GetBooleanValue(
				curTok.Features()))
			nNumCap++;
		while (!curTok.EntityType().IsEnd()) {
			++pi_Start;
			...
			if (TokenFeature.Feature.StartsWithCapital.Mechanism()
					.GetBooleanValue(curTok.Features()))
				nNumCap++;
		}
		return nNumCap;
	}
