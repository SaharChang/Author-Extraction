...
public class LexiconOnDisk implements LexiconOnDiskI, Serializable {
	...
	public static enum Lexicon {
		...
		CLOSED_SOURCE_LEXICON
		// Use closed-source lexicons covering all entity types
	}
	...
	public LexiconOnDisk(Lexicon pi_LexisonSet) {
		...
		if (pi_LexisonSet == Lexicon.OPEN_SOURCE_LEXICON) {
			ReadLexicon(cl, "lexicon/NERF-association.txt",
					NamedEntityTypeEnum.association);
			ReadLexicon(cl, "lexicon/NERF-celebrity.txt",
					NamedEntityTypeEnum.celebrity);
			ReadLexicon(cl, "lexicon/NERF-city.txt", NamedEntityTypeEnum.city);
			ReadLexicon(cl, "lexicon/NERF-company.txt",
					NamedEntityTypeEnum.company);
			ReadLexicon(cl, "lexicon/NERF-company_designator.txt",
					NamedEntityTypeEnum.company_designator);
			ReadLexicon(cl, "lexicon/NERF-country.txt",
					NamedEntityTypeEnum.country);
			ReadLexicon(cl, "lexicon/NERF-first_name.txt",
					NamedEntityTypeEnum.first_name);
			ReadLexicon(cl, "lexicon/NERF-government.txt",
					NamedEntityTypeEnum.government);
			ReadLexicon(cl, "lexicon/NERF-last_name.txt",
					NamedEntityTypeEnum.last_name);
			ReadLexicon(cl, "lexicon/NERF-military.txt",
					NamedEntityTypeEnum.military);
			ReadLexicon(cl, "lexicon/NERF-month.txt", NamedEntityTypeEnum.month);
			ReadLexicon(cl, "lexicon/NERF-person_title.txt",
					NamedEntityTypeEnum.person_title);
			ReadLexicon(cl, "lexicon/NERF-state_province.txt",
					NamedEntityTypeEnum.state_province);
		}
	 ...
	 *            types
	 */
	protected void ReadLexicon(ClassLoader pi_CL, String pi_Filename,
			NamedEntityTypeEnumI pi_Type) {
		try {
			//TODO
			InputStream in = new FileInputStream(pi_Filename);// pi_CL.getResourceAsStream(pi_Filename);
			BufferedReader inFile = new BufferedReader(
					new InputStreamReader(in));
			ReadLexicon(inFile, pi_Type);
		} catch (Exception e) {
			System.out.println("problem with "+ pi_Filename);
//			System.out.println(e.getMessage());
	...
	protected void ReadLexicon(String pi_Filename, NamedEntityTypeEnumI pi_Type) {
		try {
			BufferedReader inFile = new BufferedReader(new InputStreamReader(
					new FileInputStream(pi_Filename)));
			ReadLexicon(inFile, pi_Type);
		} catch (Exception e) {
	...
	private void ReadLexicon(BufferedReader pi_File,
			NamedEntityTypeEnumI pi_Type) {
		try {
			String line = pi_File.readLine();
			while (line != null) {
				if (!line.equals("\n") && !line.equals("\n\r")
						&& !line.equals("p") && !line.equals("/")) {
	...
	protected void AddEntityToLexicon(String pi_Line,
			NamedEntityTypeEnumI pi_Type) {
		...
		if (subMap.containsKey(pi_Line)) {
			...
			curType.AddType(pi_Type, new NamedEntityExplanation(
					NamedEntityExplanation.ExplanationType.LEXICON_LOOKUP,
					new Object[] { pi_Line, pi_Type.Label() }));
	...
	private boolean EntityIsInLexicon(String pi_Word) {
		if (pi_Word == null || pi_Word.length() == 0)
			return false;
		Hashtable<String, NamedEntityType> subMap;
	...
	private NamedEntityType GetTypeFromLexicon(String pi_Word) {
		...
		if (m_Entity.containsKey(StrFirstLetter)) {
			...
			throw new Error(
					"Cannnot get if the entity is not contained in the lexicon");
		return subMap.get(pi_Word);
	}

	/**
	 * Check if a given word is listed in a lexicon and return it (or its fuzzy
	 * variant found in the lexicon)
	 * 
	 * @param pi_Word
	 *            a word (can be a phase)
	 * @return the word found in the lexicon (either the input word or one of
	 *         its fuzzy variant) returns NULL if not found!
	 */
	...
	/**
	 * Get the entity type for this word. Entity type is a bit flag made of one
	 * or many entity types (see {@link TokenConsts} for type list)
	 * 
	 * @param pi_Word
	 * @return the type(s)
	 * @see TokenConsts
	 */
	// TODO: could reuse fuzzy match done in "GetEntityInLexicon" instead of
	...
	public NamedEntityType GetEntityType(String pi_Word, boolean pi_bFuzzy) {
		...
		if (!pi_Word.equals(strMatchWord)) {
			net = GetTypeFromLexicon(strMatchWord).clone(
					new NamedEntityExplanation(
							NamedEntityExplanation.ExplanationType.FUZZY_MATCH,
							new Object[] { pi_Word, strMatchWord }));
		} else {
	...
	private ArrayList<String> FuzzyVariants(String pi_Word) {
		...
		String strNoAccent = TokenFeature.Feature.GetStrippedAccent.Mechanism()
				.GetNominalValue(tf, m_AccentLookup);
		if (!strNoAccent.equals(pi_Word))
			alVariants.add(strNoAccent);
		...
		if (pi_Word.length() >= Balie.MIN_SIZE_FOR_FUZZY_MATCH) {
			...
			for (int i = pi_Word.length() - 1; i > 3; --i) {
				...
				if (i < pi_Word.length() - 2
						|| !m_PunctLookup.IsPunctuation(pi_Word.charAt(i))) {
					strTruncatedWord = pi_Word.substring(0, i)
							+ pi_Word.substring(i + 1, pi_Word.length());
					alVariants.add(strTruncatedWord);
					if (alVariants.size() >= Balie.MAX_NUMBER_FUZZY_VARIANTS)
						break;
				}
			}
		}
